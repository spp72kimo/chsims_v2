...
# 計算 Simple Average
# ============================================================
test_df["pred_avg"] = (test_df["pred_v"] + test_df["pred_a"] + test_df["pred_t"]) / 3
...
def calculate_metrics(pred, label):
    ...
    # 2. F1_score (排除 label=0)
    if acc2_mask.sum() > 0:
        pred_bin = pred_binary
...
print("  - Simple Average: (pred_v + pred_a + pred_t) / 3 vs y_true")
...
print(
    f"  - Acc2 (排除 y_true=0): {avg_metrics['Acc2']:.4f} (N={avg_metrics['Acc2_N']})"
)
